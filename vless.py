# vless.py – VLESS Relay logic (unchanged logic, translated comments)
# Modified to use real client IP (X‑Forwarded‑For) and English strings.
...
def _ws_client_ip(ws: WebSocket) -> str:
    """Extract real client IP from WebSocket headers."""
...
async def parse_vless_header(chunk: bytes):
    """Parse VLESS header to get target address and port."""
    if len(chunk) < 24:
...
async def check_and_use(uid: str, n: int) -> bool:
    """Update usage statistics and check quota/active status."""
